...
async function createToken(env) {
  const payload = textBase64Url(JSON.stringify({ iat: Date.now() }));
...
async function api(request, env) {
  ...
  if (url.pathname === "/api/admin/login" && request.method === "POST") {
    ...
    if (String(data.password || "") !== await getPassword(env)) {
  ...
  if (url.pathname === "/api/links" && request.method === "POST") {
    ...
    const link = {
      ...
      url: parsed.toString(),
      source: clean(data.source || "Loja", 60),
      category: clean(data.category || "Diversos", 60),
      price: clean(data.price, 30),
      image: clean(data.image, 500),
...
export default {
  async fetch(request, env) {
    ...
    // Tudo que não for API continua sendo servido como arquivo estático pelo Pages.
